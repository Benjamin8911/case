<template>
    <div class="import-privatekey">
        <p class="label">输入私钥</p>
        <textarea class="enter-area" placeholder="请输入明文私钥" v-model="privateKey" tabindex="-1"></textarea>
        ...
        <div class="bottom-wrapper">
            ...
            <button-item title="开始导入" :disabled="isDisabled" v-tap="{ methods:importPrivateKey }"></button-item>
            <div class="question" v-tap="{ methods: jump, path: 'question', question: 'privateKey' }">{{$t('m.whatPrivate')}}？</div>
        </div>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            isDisabled () {
                return this.privateKey === '' || this.checkPassword || this.identifyPassword || !this.radioCheckOne
            }
        },
        methods: {
            checkAgreement () {
            ...
            importPrivateKey () {
                ...
                if (this.privateKey.length !== 64) {
                    MessageBox.close()
                    MessageBox(this.$t('m.tips'), this.$t('m.privateError'))
                    return
                }
                plusReady(() => {
                ...
                let param = {
                    ...
                    privateKey: encodeURIComponent(encryptByKey(this.privateKey, ticket)),
                    ticket: ticket
                }
                remote.find_wallet_by_private(param).then(response => {
                    // 导入成功
