...
export default {
  ...
  methods: {
    ...
    fetchPrimary () {
      wx.request({
        method: 'GET',
        url: 'https://api.myoffer.cn/svc/mo-eva/rookie/question',
        dataType: 'json',
        success: (res) => {
    ...
    submitAnswer () {
      wx.request({
        method: 'POST',
        url: 'https://api.myoffer.cn/svc/mo-eva/rookie/submit',
        data: {
          redirect: 'applet',
        ...
        success: (res) => {
          ...
          if (result.code === 0) {
            ...
            if (ifPhoneExist) {
              wx.navigateTo({
                url: '/pages/result/main?id=' + result.result.id + '&from=primary'
              })
            } else {
              wx.navigateTo({
                url: '/pages/authorization/main?id=' + result.result.id + '&from=primary'
              })
            }
          } else {
            wx.showToast({
