<template>
    <div class="wechatList">
        <h2><b class="back" @click="goback"></b>公司订阅号</h2>
        <ul class="list">
            <li class="list-item clearfix" v-for="(item,index) in wechatList" :key="index">
                ...
                <div class="btn-box fr"><button class="btn-copy" data-clipboard-text="123123">复制</button><button class="btn-code" @click="showCode(item.dataImageTop,item.dataType)">二维码</button></div>
            </li>
        </ul>
        ...
        methods:{
            ...
            fetchWechatCompany(){
                ...
                _this.$http.get(API.baseUrl+'/daikuan/api/app/officialApp/Kpl').then(res=>{
                    ...
                    if(result.code==200){
                        _this.wechatList = result.data.hasOwnProperty('4')?result.data['4']:[];
                    }
